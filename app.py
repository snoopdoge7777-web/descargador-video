...
Microservicio completo de recorte automático de YouTube.

Flujo por cada link recibido:
1. Descarga el video (yt-dlp; si YouTube lo bloquea, cae a pytubefix).
2. Detecta los cortes automáticamente por silencios/pausas de audio
   (ffmpeg silencedetect) — no hace falta indicar tiempos a mano.
3. Cada clip se numera secuencialmente: 01, 02, 03...
4. Sube cada clip a Discord (para revisar).
5. Si están configuradas las credenciales de YouTube, sube el mismo
   clip a YouTube en estado "private" (o sea, PENDIENTE — solo vos lo
   ves hasta que lo publiques).
6. Si está configurado GOOGLE_DRIVE_FOLDER_ID, también sube una copia
   a esa carpeta de Drive como backup.

Variables de entorno:
- DISCORD_WEBHOOK_URL  (obligatoria para el paso 4)
- API_TOKEN            (protege el endpoint; opcional pero recomendada)
- YOUTUBE_CLIENT_ID / YOUTUBE_CLIENT_SECRET / YOUTUBE_REFRESH_TOKEN
  (opcionales — si no están, el sistema sigue funcionando, solo que
  no sube nada a YouTube, únicamente a Discord)
- GOOGLE_DRIVE_FOLDER_ID (opcional, backup en Drive)

Body esperado (POST a "/"):
{
  "urls": ["https://youtube.com/...", "..."],
  "job_id": "opcional",
  "titulo_base": "Mi Serie",     // los clips se van a llamar "Mi Serie 01", "Mi Serie 02"...
  "max_clips": 8,                // opcional, tope de clips por link
...
# ----------------------------------------------------------------------
# Chequeo de salud / configuración (todo gratis)
# ----------------------------------------------------------------------
...
def health():
    estado = {
        "servicio": "ok",
        "hosting": "Render (free tier) — sin costo mientras no supere las horas gratis mensuales",
        "herramientas_gratis": ["yt-dlp", "pytubefix", "ffmpeg"],
    ...
    if youtube_configurado:
        try:
            ...
            estado["youtube_cuota_gratis_diaria"] = "10,000 unidades/día (~6 subidas de video/día gratis)"
        except Exception as e:
